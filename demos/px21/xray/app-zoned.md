...
class App
{
  ...
  async get()
  {
    lively.notify("ZONE0 " + Zone.current.foo)
  
    var result = await fetch("https://eno34ez0jykr9nz.m.pipedream.net/");
    var text = await result.text;
    
    lively.notify("ZONE1 " + Zone.current.foo)
    
    this.textboxElement.innerHTML = text;
    
    lively.notify("ZONE2 " + Zone.current.foo)
