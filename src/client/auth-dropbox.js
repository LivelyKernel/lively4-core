...
export default class AuthDropbox {
  ...
  static onAuthenticated(windowUuid, authInfo) {
    ...
    if (!state) { 
      throw new Error("AuthDropbox Error, no state! Authinfo: " + JSON.stringify(authInfo))
    }
    ...
    focalStorage.setItem("dropboxToken", token).then(function() {
      ...
      if (cb) {
        ...
        throw new Error("AuthDropbox Error: no callback found for" + state)
      }
    })
  }
  ...
  static popup(url) {
    ...
    if (!popupWindow) {
      this.notifyMe("Dropbox Authenfication required", "click here to authenticate", function() {
  ...
  static challengeForAuth(uuid, cb) {
    ...
    fetch("https://lively-kernel.org/lively4-auth/open_dropbox_accesstoken?state="+uuid)
