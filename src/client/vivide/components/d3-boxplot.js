...
export default class D3Boxplot extends Morph {
  ...
  display(data) {
    ...
    let boxPlot = (data, {
      ...
      labelAccessor = 0,
      dataPointAccessor = 1,
      min= data.reduce((acc, dat) => Math.min(acc, dat[dataPointAccessor].reduce((acc, num) => Math.min(acc, num), Infinity
      )), Infinity),
      max = data.reduce((acc, dat) => Math.max(acc, dat[dataPointAccessor].reduce((acc, num) => Math.max(acc, num), -Infinity
      )), -Infinity),
