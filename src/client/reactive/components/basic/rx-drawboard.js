...
import Rx from 'https://unpkg.com/rxjs@7.5.5/dist/bundles/rxjs.umd.js'
...
export default class LivelyDrawboard extends Morph {
  ...
  RX_EXAMPLE() {
    extend(new Rx.Observable().__proto__, Object.fromEntries(Rx.operators.toPairs().map(([key, operator]) => [key, function(...args) {
      return this.pipe(operator(...args))
    }])))
    
    const svg = this.svg;
    
    const down = Rx.fromEvent(svg, 'pointerdown')
    // down.subscribe(Function.noop).unsubscribe()
    const move = Rx.fromEvent(svg, 'pointermove')

    const lines = down.switchMap(() => {
      // after a mouse down, we'll record all mouse moves
      return down.merge(move)
        // operator chosen for explanatory purpose only, not usability
        // .throttleTime(1000 / 30)
        .map(::lively.getPosition)
        // we'll stop (and unsubscribe) once the user releases the mouse
        // this will trigger a 'mouseup' event    
        .takeUntil(Rx.fromEvent(svg, 'pointerup'))
        // pairwise lets us get the previous value to draw a line from
        // the previous point to the current point   
        .pairwise()
    })
    lines.subscribe(::this.renderLine)
    lines
      .debounceTime(500)
      .subscribe(() => this.persistLines())

    /*
    in general, fold over the value history of a stream
    
    */
    //1 we use a slightly adjusted API that we consider more ideomatic to \js by adding operators directly to Observables
    // 2 ReScala also possesses a lot of functionality of Rx, we use Rx due to its popularity
  }
  
  persistLines() {
    return lively.notify('PERSIST')
  }

  renderLine(...args) {
    return this.renderLineSegment(...args)
  }
  
  renderLineSegment([fromEvt, toEvt]) {
    if (this.cPath) {
      this.cPath.remove()
    }
    ...
    var eventPosFrom = fromEvt
    var eventPosTo = toEvt

    this.offset  = this.getOffset(this.svg);
    // use screenX and screenY for higher resolution
    var posFrom = eventPosFrom.subPt(this.offset)
    var posTo = eventPosTo.subPt(this.offset)
   
    path.setAttribute("d", `M ${posFrom.x} ${posFrom.y} L ${posTo.x} ${posTo.y}`)
