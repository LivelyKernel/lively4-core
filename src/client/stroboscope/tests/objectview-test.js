...
describe('object view logic', () => {
  ...
  it('create entry on new property', () => {
    ...
    expect(view.propertiesEventsMap[event1.property].length).to.equal(1)
    expect(view.propertiesEventsMap[event1.property][0]).to.equal(event1)
    expect(view.propertiesEventsMap[event2.property].length).to.equal(1)
  ...
  it('append event to entry on existing property', () => {
    ...
    expect(view.propertiesEventsMap[event1.property].length).to.equal(2)
    expect(view.propertiesEventsMap[event1.property][0]).to.equal(event1)
    expect(view.propertiesEventsMap[event1.property][1]).to.equal(event2)
  });
});
