...
export default function lineToCurveGenerator(curveType='q')
{
	...
	return function lineToCurve(segment, __, path)
	{
		...
		if(segment.type === 'l')
		{
			...
			switch(curveType)
			{
				case 'q':
				{
          document.getElementById('svgReferencesvg')?.insertAdjacentHTML?.('beforeend', `<circle cx="${segment.x}" cy="${segment.y}" r="20" fill="blue"></circle>`)
