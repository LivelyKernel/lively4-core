...
export function diffDOM(e) {
  ...
  function n(e, n, s) {
    ...
    switch (n[s._const.action]) {
      ...
      case s._const.modifyAttribute:
        if (!r || !r.setAttribute) return !1;
        r.setAttribute(n[s._const.name], n[s._const.newValue]), "INPUT" === r.nodeName && "value" === n[s._const.name] && (r.value = n[s._const.newValue]);
        break;
  ...
  function k(e, t) {
    ...
    var o, n = [],
      ...
      a = {},
      l = !1;
    return e.replace(b, function(c, r) {
      if (l) {
        if (c !== "</" + o.nodeName + ">") return;
        l = !1
      }
      var u, d = "/" !== c.charAt(1),
        h = r + c.length,
        f = e.charAt(h);
      if (d && (s++, "tag" === (o = function(e) {
          var t = {
              nodeName: "",
              attributes: {}
            },
            o = e.match(/<\/?([^\s]+?)[\/\s>]/);
          o && (t.nodeName = o[1].toUpperCase(), (E[o[1].toLowerCase()] || "/" === e.charAt(e.length - 2)) && (t.voidElement = !0));
          for (var n = new RegExp(y), s = null, i = !1; !i;)
            if (null === (s = n.exec(e))) i = !0;
            else if (s[0].trim())
            if (s[1]) {
              var a = s[1].trim(),
                l = [a, ""];
              a.indexOf("=") > -1 && (l = a.split("=")), t.attributes[l[0]] = l[1], n.lastIndex--
            } else s[2] && (t.attributes[s[2]] = s[3].trim().substring(1, s[3].length - 1));
          return t
        }(c)).type && t.components[o.nodeName] && (o.type = "component", l = !0), o.voidElement || l || !f || "<" === f || (o.childNodes || (o.childNodes = []), o.childNodes.push({
          nodeName: "#text",
          data: w(e.slice(h, e.indexOf("<", h)))
        })), a[o.tagName] = o, 0 === s && n.push(o), (u = i[s - 1]) && (u.childNodes || (u.childNodes = []), u.childNodes.push(o)), i[s] = o), (!d || o.voidElement) && (s--, !l && "<" !== f && f)) {
        u = -1 === s ? n : i[s].childNodes || [];
        var p = e.indexOf("<", h),
          m = w(e.slice(h, -1 === p ? void 0 : p));
        u.push({
          nodeName: "#text",
          data: m
        })
  ...
  A.prototype.init = function() {
    ...
    do {
      ...
      0 === (o = this.findNextDiff(e, t, [])).length && (d(e, t) || (this.foundAll ? console.error("Could not find remaining diffs!") : (this.foundAll = !0, u(e), o = this.findNextDiff(e, t, [])))), o.length > 0 && (this.foundAll = !1, this.tracker.add(o), g(e, o, this.options))
    } while (o.length > 0);
    return this.tracker.list
  }, A.prototype.findNextDiff = function(e, t, o) {
    var n, s;
    if (this.options.maxDepth && o.length > this.options.maxDepth) return [];
    if (!e.outerDone) {
      if (n = this.findOuterDiff(e, t, o), this.options.filterOuterDiff && (s = this.options.filterOuterDiff(e, t, n)) && (n = s), n.length > 0) return e.outerDone = !0, n;
      e.outerDone = !0
    }
    if (!e.innerDone) {
    ...
    return []
  }, A.prototype.findOuterDiff = function(e, t, o) {
    var n, s, i, l, c, r, u = [];
    if (e.nodeName !== t.nodeName) {
      if (!o.length) throw new Error("Top level nodes have to be of the same kind.");
      return [(new a).setValue(this.options._const.action, this.options._const.replaceElement).setValue(this.options._const.oldValue, f(e)).setValue(this.options._const.newValue, f(t)).setValue(this.options._const.route, o)]
    }
    if (o.length && this.options.maxNodeDiffCount < Math.abs((e.childNodes || []).length - (t.childNodes || []).length)) return [(new a).setValue(this.options._const.action, this.options._const.replaceElement).setValue(this.options._const.oldValue, f(e)).setValue(this.options._const.newValue, f(t)).setValue(this.options._const.route, o)];
    ...
    return u
  }, A.prototype.findInnerDiff = function(e, t, o) {
    ...
    if (!this.options.maxChildCount || i < this.options.maxChildCount) {
      var u = e.subsets && e.subsetsAge-- ? e.subsets : e.childNodes && t.childNodes ? function(e, t) {
        for (var o = e.childNodes ? e.childNodes : [], n = t.childNodes ? t.childNodes : [], s = m(o.length, !1), i = m(n.length, !1), a = [], l = !0, c = function() {
            return arguments[1]
          }; l;) {
          if (l = p(o, n, s, i)) a.push(l), Array.apply(void 0, new Array(l.length)).map(c).forEach(function(e) {
      ...
      if (u.length > 0 && (c = this.attemptGroupRelocation(e, t, u, o)).length > 0) return c
    }
    for (var h = 0; h < i; h += 1) {
      var _ = n[h],
        V = s[h];
      l && (_ && !V ? "#text" === _.nodeName ? (c.push((new a).setValue(this.options._const.action, this.options._const.removeTextElement).setValue(this.options._const.route, o.concat(r)).setValue(this.options._const.value, _.data)), r -= 1) : (c.push((new a).setValue(this.options._const.action, this.options._const.removeElement).setValue(this.options._const.route, o.concat(r)).setValue(this.options._const.element, f(_))), r -= 1) : V && !_ && ("#text" === V.nodeName ? c.push((new a).setValue(this.options._const.action, this.options._const.addTextElement).setValue(this.options._const.route, o.concat(r)).setValue(this.options._const.value, V.data)) : c.push((new a).setValue(this.options._const.action, this.options._const.addElement).setValue(this.options._const.route, o.concat(r)).setValue(this.options._const.element, f(V))))), _ && V && (!this.options.maxChildCount || i < this.options.maxChildCount ? c = c.concat(this.findNextDiff(_, V, o.concat(r))) : d(_, V) || (n.length > s.length ? (c = c.concat([(new a).setValue(this.options._const.action, this.options._const.removeElement).setValue(this.options._const.element, f(_)).setValue(this.options._const.route, o.concat(r))]), n.splice(h, 1), r -= 1, l -= 1) : n.length < s.length ? (c = c.concat([(new a).setValue(this.options._const.action, this.options._const.addElement).setValue(this.options._const.element, f(V)).setValue(this.options._const.route, o.concat(r))]), n.splice(h, 0, {}), l -= 1) : c = c.concat([(new a).setValue(this.options._const.action, this.options._const.replaceElement).setValue(this.options._const.oldValue, f(_)).setValue(this.options._const.newValue, f(V)).setValue(this.options._const.route, o.concat(r))]))), r += 1
    }
    return e.innerDone = !0, c
  }, A.prototype.attemptGroupRelocation = function(e, t, o, n) {
    for (var s, i, l, c, r, u, d = function(e, t, o) {
      ...
      }(e, t, o), p = d.gaps1, _ = d.gaps2, V = Math.min(p.length, _.length), g = [], v = 0, b = 0; v < V; b += 1, v += 1)
      if (!0 === p[v])
        if ("#text" === (c = e.childNodes[b]).nodeName) {
          if ("#text" === t.childNodes[v].nodeName && c.data !== t.childNodes[v].data) {
            for (u = b; e.childNodes.length > u + 1 && "#text" === e.childNodes[u + 1].nodeName;)
      ...
      if (l = o[p[v]], (i = Math.min(l.newValue, e.childNodes.length - l.length)) !== l.oldValue) {
        s = !1;
        for (var N = 0; N < l.length; N += 1) h(e.childNodes[i + N], e.childNodes[l.oldValue + N], [], !1, !0) || (s = !0);
    ...
    return g
  }, A.prototype.findValueDiff = function(e, t, o) {
    var n = [];
  ...
  var D = {
    ...
    O = function(e) {
      var t = this;
      if (void 0 === e && (e = {}), this.options = e, Object.entries(D).forEach(function(e) {
          var o = e[0],
            n = e[1];
  ...
  return T.prototype.fin = function(e, t) {
    this.padding += this.pad, this.log("├─> entering " + e, t)
  }, T.prototype.fout = function(e, t) {
  ...
  }, e.DiffDOM = O, e.TraceLogger = T, e.nodeToObj = v, e.stringToObj = x, e
}({});
