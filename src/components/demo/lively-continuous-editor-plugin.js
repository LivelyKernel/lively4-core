...
export default function(babel) {
  ...
  let checkRuntime = template(
    'if (performance.now() - _tr_time > _tr_time_max) throw new Error("Run to long! Endless loop?");')
  ...
  return {
    ...
    visitor: {
      Program(path) {
        ...
        )({ ASTID: t.stringLiteral(programast.astid) }));
        ...
        functions.forEach(ea => {
          ...
          body.replaceWith(template("{ return _tr_( NODEIDX , () => BLOCK)}")({
            NODEIDX: t.numericLiteral(ea.node.traceid),
            BLOCK: body.node
          }))
        });
