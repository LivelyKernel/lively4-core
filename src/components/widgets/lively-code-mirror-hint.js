...
class CompletionsBuilder {
  ...
  async buildHint(options) {
    ...
    const getCompletionsFromImports = async () => {
      ...
      imports.map(i => forOrigin(i, 'import')).forEach(maybeAdd);
    };

    const autocompleteImport = async cm => {
    ...
    const javascriptHint = async (cm, options) => {
      ...
      await getCompletions(token, context, options);
    };

    // #CodeMirror #AutoCompletion happens here!
    const getCompletions = async (token, context, options) => {
      ...
      const prevToken = cm.getTokenAt({ line: cursor.line, ch: token.start - 0 });
      const prevPrevToken = cm.getTokenAt({ line: cursor.line, ch: token.start - 1 });
      ...
      for (var v = token.state.globalVars; v; v = v.next) {
        maybeAdd(forOrigin(v.name, 'global'));
      }
      if (!options || options.useGlobalScope !== false) {
    ...
    await javascriptHint(cm, options);
    lively.notify(completions.list.length, 'num completions');
    return completions.asCompletionsObject();
  }
