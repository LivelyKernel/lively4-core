...
export default class Editor extends Morph {
  ...
  toggleVersions() {
    ...
    if (this.versionControl.style.display == "block") {
      ...
      lively.setGlobalPosition(this.versionControl, lively.getGlobalPosition(this).addPt(pt(lively.getExtent(this.parentElement).x, 0)));
        ...
        if (item.type == 'lively/element') {
          ...
          item.getAsString(data => {
            ...
            if (element.localName == "lively-file") {
              this.pasteDataUrlAs(element.url, this.getURLString().replace(/[^/]*$/, "") + element.name, element.name, evt);
            }
    ...
    if (type == "js") {
      for (let m of Strings.matchAll(/\/\*((?:HTML)|(?:MD))(.*?)\1\*\//, codeMirrorComponent.value)) {
        ...
        //           })
          ...
          if (container) {
            lively.html.fixLinks(widget.shadowRoot.querySelectorAll("[href],[src]"), this.getURL().toString().replace(/[^/]*$/, ""), url => container.followPath(url));
          }
        } else {
          widget.innerHTML = m[2];
          let container = lively.query(this, "lively-container");
          if (container) {
            lively.html.fixLinks(widget.querySelectorAll("[href],[src]"), this.getURL().toString().replace(/[^/]*$/, ""), url => container.followPath(url));
          }
        }
      }
    }
  ...
  async hideEmbeddedWidgets() {
    var codeMirrorComponent = this.get("lively-code-mirror");
    if (!codeMirrorComponent) return;
    codeMirrorComponent.editor.doc.getAllMarks().filter(ea => ea.widgetNode && ea.widgetNode.querySelector(".lively-widget")).forEach(ea => ea.clear());
  }
  ...
  toggleWidgets() {
    ...
    var allWidgets = codeMirrorComponent.editor.doc.getAllMarks().filter(ea => ea.widgetNode && ea.widgetNode.querySelector(".lively-widget"));
