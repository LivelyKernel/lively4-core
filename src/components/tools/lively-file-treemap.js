...
export default class LivelyFileTreemap extends LivelyD3Treemap {
  ...
  async initialize() {
    super.initialize();
    super.setTreeData(await Files.fileTree('src'));
    this.addEventListener('extent-changed', ((evt) => { this.onExtentChanged(evt);})::super.debounce(500));

  }

  
  /*async initialize() {
    this.windowTitle = "LivelyFileTreemap";
