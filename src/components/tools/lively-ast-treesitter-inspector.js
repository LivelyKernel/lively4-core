...
export default class AstTreesitterInspector extends AstInspector {
  ...
  renderTreeSitterNode(element, expanded) {
    ...
    element.append(<span> <span style="font-size:6pt;padding:0px 2px 0px 2px;margin:0px; border-radius:3px; background:gray; color:yellow"> {target.id - target.tree.rootNode.id}</span></span>);
    element.append(<span> <span style="font-size:6pt;padding:0px;margin:0px">{target.startIndex}-{target.endIndex} </span></span>);
